...
Геттеры и сеттеры в JS - это аксессорные свойства. В отличие от обычного свойства, которое хранит value, аксессор хранит функции get/set.
При чтении `obj.prop` движок вызывает `getter`, при записи — `setter`.
> То есть obj.prop — это синтаксический сахар: движок либо возвращает value, либо вызывает get.

**Важно:**
- У accessor property нет value и writable.
- Когда ты пишешь obj.prop = x, сеттер может:
  - записать в “приватное” поле (_prop)
  - валидировать
  - триггерить сайд-эффекты (лог, события, кеш, пересчёт)
  
Если есть только get, запись obj.prop = x:
в strict-mode обычно бросит TypeError (зависит от контекста/наследования), либо просто не сработает.<br>
Если есть только set, чтение obj.prop даст undefined (нет get).<br>
Если в set ты пишешь в это же свойство, получишь рекурсию:<br>
```js
set age(v){ this.age = v } // бесконечный вызов set
// Поэтому пишут в backing-field: this._age = v.
```

## На собесах любят: “как работает this/receiver”.

```js
function User(age) { this._age = age; }
...
const u = new User(10);
console.log(u.age); // getter, this = u
```
**Чтение u.age — как движок ищет свойство**:
1. Ищем собственное свойство "age" у u. Его нет.
2. Идём в прототип: User.prototype. Там "age" есть, и это аксессор.
3. Раз это аксессор и есть get, движок вызывает getter.

> Ключевой момент: с каким this вызывается getter? <br>
this = receiver, то есть объект, на котором был доступ: u.
Поэтому внутри getter:
```js
get() { return this._age; }
```
`this` — это `u`, значит читается `u._age`, который равен `10`.

Результат: в консоль выводится 10.

**Если сделать:**
```js
u.age - 20;
```
1. У u нет собственного "age", идём в прототип.
2. Находим аксессор "age" с set.
3. Вызываем setter с this = u, v = 20.
4. Setter делает: ```u._age = 20; ```
> То есть age - это “публичное API”, а реальное хранение - в _age.

>Почему this = u, а не User.prototype?

>Потому что this в getter/setter — это объект-получатель (receiver), на котором происходит операция доступа/присваивания.
>Это специально сделано, чтобы аксессоры на прототипе работали корректно для всех инстансов.

>**Геттеры/сеттеры нужны, чтобы сделать доступ к полю “как к свойству”, но под капотом выполнять логику. ИКАПСУЛЯЦИЯ** 

# Proxy

## Что такое Proxy? Для чего он нужен? Когда он может быть полезен?
Это объект обертка, которая позволяет перехватывать и переопределять действия над целевым объектом. (чтеине, запись, удаление).
```js
const proxy = new Proxy(target, handler)
```
- target – это объект, для которого нужно сделать прокси, может быть чем угодно, включая функции
- handler - объект с ловушками, например, ловушка get – для чтения свойства из target, ловушка set – для записи свойства в target и так далее.
```js
let target = {};
...
- get(target, prop, receiver) — чтение: obj.x, obj['x'], доступ к геттеру.
- set(target, prop, value, receiver) — запись: obj.x = 1.
- has(target, prop) — проверка наличия: 'x' in obj.
- deleteProperty(target, prop) — удаление: delete obj.x.
- ownKeys(target) — перечисление ключей: Object.keys, Reflect.ownKeys, for...in (в части получения списка).
...
Это жёсткие правила “нельзя врать про target”. Можно перехватить операции, но результаты ловушек не должны противоречить состоянию объекта.
Самые частые случаи:
-Нельзя “спрятать” non-configurable свойство (например, в ownKeys или has).
-Если объект non-extensible, нельзя делать вид, что у него есть новые свойства или не хватает существующих.
-Нельзя “успешно” записать в свойство с writable: false (trap set не может вернуть true, если запись невозможна).
-Нельзя “успешно удалить” non-configurable свойство (deleteProperty не может вернуть true).
- Для non-extensible объекта нельзя “выдумать” другой прототип (getPrototypeOf).
> non-extensible объект — это объект, который нельзя расширять, то есть нельзя добавлять новые свойства. <br><br>
Как делают:
Object.preventExtensions(obj) → делает объект non-extensible
Проверка: Object.isExtensible(obj) → false
```js
const o = { a: 1 };
Object.preventExtensions(o);

o.b = 2;           // в strict mode -> TypeError, иначе просто не добавится
console.log(o.b);  // undefined
```
## Что такое Reflect и для чего он нужен?
Внутри движка есть набор “внутренних операций” над объектами: прочитать свойство, записать, удалить, узнать ключи, вызвать функцию, сделать new, поменять прототип, запретить расширение и т.д.
<br><br>`Reflect` — это прямой, стандартный способ вызвать эти операции как функции.
1. Удобнее и безопаснее в Proxy
В ловушках Proxy почти всегда делают “дефолтное поведение” через Reflect, чтобы:
не забыть тонкости (receiver в get/set)
не нарушить инварианты
корректно вернуть true/false
```js
const p = new Proxy(obj, {
  get(t, prop, r) {
    console.log("get", prop);
    return Reflect.get(t, prop, r); // правильное стандартное чтение
  },
  set(t, prop, val, r) {
    if (prop === "age" && val < 0) return false;
    return Reflect.set(t, prop, val, r); // вернёт boolean
  }
});

```

2 Единый стиль вместо смеси операторов и Object.*
- Reflect.get(obj, "x") ↔ obj.x
- Reflect.has(obj, "x") ↔ "x" in obj
- Reflect.deleteProperty(obj, "x") ↔ delete obj.x
- Reflect.ownKeys(obj) ↔ Object.getOwnPropertyNames + getOwnPropertySymbols
> Reflect — это стандартный API для “низкоуровневых” операций над объектами (get/set/has/delete/defineProperty/ownKeys, apply/construct и т.д.). Он нужен, чтобы вызывать эти операции как функции с предсказуемыми возвращаемыми значениями и корректной семантикой. Главный кейс — Proxy: внутри traps обычно делают return Reflect.*(...), чтобы сохранить поведение по умолчанию и не нарушить инварианты.

## Что такое Reciever 
> receiver = объект “слева от точки” (или Proxy), который должен стать this для getter/setter и корректно участвовать в записи.
```js
const p = new Proxy(obj, {
  get(t, prop) {
    return t[prop]; // ❌ может сломать this в геттере
  }
});

const p = new Proxy(obj, {
  get(t, prop, receiver) {
    return Reflect.get(t, prop, receiver); // ✅ сохраняет правильный this
  }
});
```
Если свойство — accessor (get/set) и оно найдено в прототипе, то this внутри accessor должен быть объект, через который обратились (тот самый receiver), а не обязательно target/прототип.